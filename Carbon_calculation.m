...
flag=1;
Year_number=1;
Carbon_Year=zeros(4,1);
Output_Year=zeros(4,4);
ES_capacity=zeros(1,4);
%% CG NG PV TL EL aerosol  
...
Change_trend=[1  Change_trend_fluctuation*1.30  Change_trend_fluctuation*1.09  Change_trend_fluctuation*0.74
              1  Change_trend_fluctuation*1.9   Change_trend_fluctuation*3     Change_trend_fluctuation*3.9
              1  Change_trend_fluctuation*2.87  Change_trend_fluctuation*4.94  Change_trend_fluctuation*6.56             
              1  Change_trend_fluctuation*1.2   Change_trend_fluctuation*1.4   Change_trend_fluctuation*1.6
              1  Change_trend_fluctuation*1.49  Change_trend_fluctuation*1.74  Change_trend_fluctuation*1.83
              1  Change_trend_fluctuation*1.03  Change_trend_fluctuation*1.06  Change_trend_fluctuation*1.09];% 

%% Unit MW/MWh/1000ton
%% plum rain affected area ratio and impact degree
province_index = input('Enter a number: ');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Considering_impact_index=input('Enter 1 (consideing plum rain impact) or 0 (not consideing plum rain impact): ');
...
for w=1:5
Cons=Cons+(P_PV1((w-1)*168+1:w*168,1)+curtailment_PV1((w-1)*168+1:w*168,1)==unit_PV_output1((w-1)*168+1:w*168,1)*(1+degree(1,w))*(1+aerosol)*C_PV*AAS);% 梅雨影响光伏发电
end
Cons=Cons+(P_PV2(:,1)+curtailment_PV2(:,1)==unit_PV_output2(:,1)*(1+aerosol)*C_PV*(1-AAS));% 梅雨不影响光伏发电
...
for t=1:T
    Cons=Cons+(sum(P_CG(t,:))+sum(P_NG(t,:))+P_RG+P_PV1(t,1)+P_PV2(t,1)+unit_WT_output(t,1)*C_WT+P_TL-P_ES_cha(t,1)+P_ES_dis(t,1)==electric_load(t,1)-curtailment_load(t,1));%改动
if t==1    
...
%% Output of carbon emissions, differenet generations, and ES_capacity
Carbon=unit_carbon_coal*(sum(sum(P_CG)))+unit_carbon_gas*(sum(sum(P_NG)));
Carbon_Year(Year_number,1)=value(Carbon);
Output_Year(Year_number,1)=value(sum(sum(P_CG)));
Output_Year(Year_number,2)=value(sum(sum(P_NG)));
Output_Year(Year_number,3)=value(sum(sum(curtailment_load)));
Output_Year(Year_number,4)=value(sum(P_PV1));
ES_capacity(1,Year_number)=value(Capacity_ES);
if Year_number>=4
    flag=0;
end
