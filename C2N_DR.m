...
%% DR compensation cost and energy
LCCM=zeros(5,5);
compensating_energy=zeros(5,5);
unit_DR=[0.25 0.5 0.75 1 1.25]';
max_DR=[0.02 0.04 0.06 0.08 0.1]';
...
%% Constraints of DR
Energy_DR=sdpvar(T,1);
Cons=Cons+(0<=Energy_DR(:,1)<=max_DR(n,1)*max(electric_load));%%%%%
...
for t=1:T
    Cons=Cons+(sum(P_CG(t,:))+sum(P_NG(t,:))+P_RG+P_PV1(t,1)+P_PV2(t,1)+unit_WT_output(t,1)*C_WT+P_TL-P_ES_cha(t,1)+P_ES_dis(t,1)+Energy_DR(t,1)==electric_load(t,1)-curtailment_load(t,1));%¸Ä¶¯
if t==1    
